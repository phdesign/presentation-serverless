...
So first, what is Serverless? The term serverless can be used in reference to the Serverless Framework, which is a toolset for managing AWS lambda function, we'll look at that framework late but we're going to be using the term to describe the more general Serverless computing or Serverless architecture.
...
By only executing as needed to respond to an event, serverless code can be very cheap to run. For AWS they bill per request and per 100ms your code runs for and per allocated memory. Currently the pricing is $0.20 per 1 million requests and $0.00001667 (0.002c) per GB-second. You also get 1 million free requests and 400,000 free GB-seconds per month. As an example, for 3 million requests to an AWS serverless function, that runs for 1 sec and uses 512MB of memory, it would cost approximately $18.34 per month.
...
To understand why serverless computing is such a revolution, it's useful to review how it's come about with a quick history Cloud computing some
...
All of them have very similar offerings, but we're going to focus a bit more on AWS.

AWS Lambda's can be configured to respond to any of the following events, and list is continuously growing. API Gateway is RESTful web request proxy. You can configure API endpoints that fire a Lambda when a request is received. S3 is AWS's file storage system. 
